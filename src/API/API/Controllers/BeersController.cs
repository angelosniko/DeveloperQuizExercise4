...
namespace API.Controllers
{
    ...
    public class BeersController : ControllerBase
    {
        ...
        public ActionResult<IEnumerable<BeerDto>> GetBeers()
        {
            ...
            var beersFromRepo = _beerLibraryRepository.GetBeers();


            return Ok(_mapper.Map<IEnumerable<BeerDto>>(beersFromRepo));
        ...
        [HttpGet("{beerId}/Average")]
        public ActionResult<BeerDto> GetAverage(Guid beerId)
        {

            var beerFromRepo = _beerLibraryRepository.GetAverage(beerId);

            var beers = new BeerDto() {Id=beerFromRepo.Id,BeerName=beerFromRepo.BeerName,BeerType=beerFromRepo.BeerType,Rating=beerFromRepo.BeerRatings.Select(g=>g.BeerRatingValue).Average() };
